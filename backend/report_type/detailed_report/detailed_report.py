...
class DetailedReport:
    def __init__(
        ...
        self.seres_researcher = SeresResearcher(
            query=self.query,
    ...
    async def run(self) -> str:
        ...
        report_introduction = await self.seres_researcher.write_introduction()
        _, report_body = await self._generate_subtopic_reports(subtopics)
        self.seres_researcher.visited_urls.update(self.global_urls)
        report = await self._construct_detailed_report(report_introduction, report_body)
        return report

    async def _initial_research(self) -> None:
        await self.seres_researcher.conduct_research()
        self.global_context = self.seres_researcher.context
        self.global_urls = self.seres_researcher.visited_urls

    async def _get_all_subtopics(self) -> List[Dict]:
        subtopics_data = await self.seres_researcher.get_subtopics()
    ...
    async def _get_subtopic_report(self, subtopic: Dict) -> Dict[str, str]:
        ...
        subtopic_assistant = SeresResearcher(
            ...
            agent=self.seres_researcher.agent,
            role=self.seres_researcher.role,
            tone=self.tone,
            complement_source_urls=self.complement_source_urls,
            source_urls=self.source_urls
        )
        ...
        parse_draft_section_titles = self.seres_researcher.extract_headers(draft_section_titles)
        ...
        self.global_written_sections.extend(self.seres_researcher.extract_sections(subtopic_report))
        self.global_context = list(set(subtopic_assistant.context))
        self.global_urls.update(subtopic_assistant.visited_urls)

        self.existing_headers.append({
            "subtopic task": current_subtopic_task,
            "headers": self.seres_researcher.extract_headers(subtopic_report),
        })
    ...
    async def _construct_detailed_report(self, introduction: str, report_body: str) -> str:
        toc = self.seres_researcher.table_of_contents(report_body)
        conclusion = await self.seres_researcher.write_report_conclusion(report_body)
        conclusion_with_references = self.seres_researcher.add_references(
            conclusion, self.seres_researcher.visited_urls)
